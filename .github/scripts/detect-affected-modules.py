...
RE_ADD_LIB = re.compile(r"(?:velox_add_library|add_library)\s*\(\s*(\w+)", re.MULTILINE)

RE_ADD_EXE = re.compile(r"add_executable\s*\(\s*(\w+)", re.MULTILINE)
...
def compute_metrics(
    ...
    affected_tests = {t for t in all_affected if t in targets and targets[t]["is_test"]}
...
def format_comment(metrics: dict, changed_files: list[str]) -> str:
    ...
    pct_libs = f"{n_affected_libs / n_total_libs * 100:.1f}%" if n_total_libs else "N/A"
    pct_tests = (
...
def main():
    ...
    print(
        f"Parsed {len(cmake_data['targets'])} targets from "
