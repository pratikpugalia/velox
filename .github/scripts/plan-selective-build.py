...
def decide_mode(changed_files: list[str]) -> str:
    ...
    Returns one of:
      - 'full'  — files outside the dependency graph changed (vendored /
                  experimental code, or a build-driver file like Makefile);
                  a full build is required regardless of the cached graph.
    ...
    """
    if any(
        f in FULL_BUILD_FILES or f.startswith(FULL_BUILD_PREFIXES)
