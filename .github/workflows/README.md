...
| Detect Force-Full Trigger | `detect-force-full.yml` | PR approving review or `/full-build` PR comment | Step 1: trivial trigger for the workflow_run hand-off |
| Rerun Linux Build on Force-Full Trigger | `rerun-on-force-full.yml` | workflow_run (after Detect Force-Full Trigger) | Step 2: BASE-context, dedups, cancels + re-runs the in-flight selective build (preserves PR head SHA on check_runs) |
...
### Auto-trigger full build (`detect-force-full.yml` + `rerun-on-force-full.yml`)

A two-workflow chain that escalates an in-flight selective build to a full build, even for fork PRs. Two events fire it:
...
The chain exists because GitHub does not grant `actions: write` to workflows triggered by fork-PR events (`pull_request_review` and `issue_comment` both included). The only way to land write-capable context — needed to cancel and re-run via the actions API — is to chain through `workflow_run`, which always runs in BASE context.

- **`detect-force-full.yml`** fires on `pull_request_review: [submitted]` (gated to `state == 'approved'`) or `issue_comment: [created]` (gated to a PR comment whose body contains `/full-build`). Trivial — exists only to fire `workflow_run`.
- **`rerun-on-force-full.yml`** is `workflow_run`-triggered. Runs in BASE context with `actions: write`, re-derives PR identity from `workflow_run.id` (GitHub-set, not forgeable), dedups by upstream event type (approvals: count == 1 on current SHA; `/full-build`: count == 1 comment on the PR), then finds the most recent `Linux Build using GCC` run on the PR head SHA, cancels it if in flight, and re-runs it via `POST /actions/runs/{id}/rerun`.
...
- **`workflow_run` pattern**: the upstream workflow uploads results as artifacts, and a separate `workflow_run`-triggered workflow downloads them in BASE context (with secrets and write `GITHUB_TOKEN`) to do the privileged work. Used by:
  ...
  - `rerun-on-force-full.yml` (chained from `detect-force-full.yml`; cancels and re-runs the in-flight `Linux Build using GCC` run on first approval or first `/full-build` comment, escalating it from selective to full)
- **API re-run from privileged context**: `rerun-on-force-full.yml` uses `POST /actions/runs/{id}/rerun` (covered by `GITHUB_TOKEN`'s `actions: write` in `workflow_run` context) instead of dispatching a fresh build. A re-run preserves the original `pull_request` event's context, including `github.sha = PR head SHA`, so check_runs land on the PR — visible to reviewers and respected by branch protection — without needing an external identity or `workflow_dispatch`/`workflow_call` plumbing.
