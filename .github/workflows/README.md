...
|----------|------|----------|---------|
| Linux Build using GCC | `linux-build.yml` + `linux-build-base.yml` | push to main, PRs, workflow_call from approval bot, manual workflow_dispatch | Main build & test (3 configs); selective by default, full on push / sticky-approval / approval-bot dispatch |
| Detect Approval | `detect-approval.yml` | PR approving review | Step 1 of fork-PR-safe approval chain: records PR meta to artifact (no secrets needed) |
| Trigger Full Build on Approval | `trigger-full-on-approval.yml` | workflow_run (after Detect Approval) | Step 2: BASE-context, dedups, calls `linux-build.yml` via workflow_call in full mode |
...
The main CI workflow for Velox. Triggered on pushes to `main`, on pull request `opened`/`synchronize`/`reopened` events, via `workflow_call` from the approval-bot chain, and via `workflow_dispatch` for maintainer-driven manual runs. The entry point `linux-build.yml` first calls the `selective-build-plan.yml` reusable workflow to decide the build mode (see [Selective Build Plan](#selective-build-plan-selective-build-planyml)), then delegates to `linux-build-base.yml` which can build and test up to three configurations in parallel on 32-core Ubuntu runners:

- **Linux adapters release** — Release build using the `velox-dev:adapters` Docker image. Enables cloud storage adapters (S3, GCS, ABFS, HDFS), Parquet, Arrow, geospatial functions, and GPU support (WAVE, cuDF). Tests run with `ctest -j 24` and a 900-second timeout. When cuDF-related files change, a separate cuDF test job runs on a `4-core-ubuntu-gpu-t4` GPU runner. **Always runs**: full build in full mode (push to main, sticky-approved PR, approval-bot dispatch, manual workflow_dispatch), targeted build (only the cmake targets affected by the PR) in selective mode. The mode and target list come from the selective-build plan. The same cmake configure runs in both modes (`VELOX_MONO_LIBRARY=OFF` so per-target builds actually save work), so post-approval re-runs are largely incremental on top of pre-approval work via ccache.
- **Ubuntu debug** — Debug build using the `velox-dev:ubuntu-22.04` Docker image. Enables benchmarks, examples, Arrow, geospatial, Parquet, shared library (`VELOX_BUILD_SHARED=ON`), and mono library modes. Tests run with `ctest -j 24` and a 1800-second timeout. **Only runs in full mode** (push to main, sticky-approved PR, approval-bot dispatch, manual workflow_dispatch). Catches debug-only assertions and validates against system Ubuntu deps.
...
In selective mode (PR without an approving review), only the `Linux release with adapters` job runs (in targeted mode), and `Ubuntu debug` and `Fedora debug` show as "Skipped" in the checks list — this is expected. The first approving review automatically triggers the full build via the [Detect Approval + Trigger Full Build on Approval](#auto-trigger-full-build-on-approval-detect-approvalyml--trigger-full-on-approvalyml) chain, which calls `linux-build.yml` via `workflow_call` with `force_full=true`. Subsequent approvals on the same head SHA are deduped in the trigger workflow and do not re-run the build.
...
- Push to `main` → full
- Caller passed `force_full=true` (approval-bot path or maintainer `workflow_dispatch`) → full
- PR with sticky approval → full (any standing approving review on the current head SHA)
...
### Auto-trigger full build on approval (`detect-approval.yml` + `trigger-full-on-approval.yml`)

A two-workflow chain that runs a full build on the first approving review, even for fork PRs. The chain exists because GitHub does not pass repository secrets to workflows triggered by *any* event tied to a fork PR — `pull_request_review` included — so a single-workflow approach can't see the PR meta needed to invoke the build. The only way to land that context in a privileged scope is to chain through `workflow_run`, which always runs in BASE context.

- **`detect-approval.yml`** fires on `pull_request_review: [submitted]`, gated to `state == 'approved'`. Runs in fork-PR context (no secrets), reads PR meta from the event payload, and uploads it as an `approval-meta` artifact. That's all it does.
- **`trigger-full-on-approval.yml`** is `workflow_run`-triggered on Detect Approval's completion. It runs in BASE context, downloads the artifact, deduplicates against earlier approvals on the same head SHA, and (if this is the first approval) calls `linux-build.yml` via `workflow_call` with `force_full=true` plus the PR meta as inputs.

The check_runs from the called `linux-build.yml` land on the PR head SHA — `workflow_call` inherits the calling workflow's run context, and `workflow_run.head_sha` is the upstream's head SHA = the PR head SHA. So branch protection sees the build, and reviewers see status on the PR.

All API calls (review count, artifact download) are read-only and `GITHUB_TOKEN` covers them in `workflow_run` context.

Dedup: the `decide` job counts approving reviews on the current head SHA via the GitHub API — count == 1 means we're the first; > 1 means a prior approval already triggered the build, so the `trigger-build` job skips. Crucially this does NOT touch the protected `Build with GCC / *` check_runs (no skipped status is created for the protected names).

### Manual full-build escape hatch

Maintainers who need to force a full build for a PR (without going through approval) can use `workflow_dispatch` on `Linux Build using GCC` from the Actions UI, supplying the PR number / head SHA / refs.
...
- **`workflow_run` pattern**: the upstream workflow uploads results as artifacts, and a separate `workflow_run`-triggered workflow downloads them in BASE context (with secrets and write `GITHUB_TOKEN`) to do the privileged work. Used by:
  ...
  - `trigger-full-on-approval.yml` (chained from `detect-approval.yml`; calls `linux-build.yml` via `workflow_call` to run a full build on first approval)
- **`workflow_call` from privileged context**: `trigger-full-on-approval.yml` invokes `linux-build.yml` via `uses:` from the BASE-context workflow_run trigger. The called workflow's check_runs land on the PR head SHA (inherited from the workflow_run's head_sha), making the build visible on fork PRs without needing an external identity to fire downstream events. The called workflow pins its `permissions:` block to read-only and explicitly does NOT receive `secrets: inherit`, so the PR-controlled build code runs with no elevated capabilities.
